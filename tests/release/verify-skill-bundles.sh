...
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F 'auth init --profile <profile> --as user --output json' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F '不得执行 `auth login --profile <profile> --as user`' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" SKILL.md | grep -F '豆包的“本地电脑”模式仍属于豆包' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F 'SESSION_ID=<same-session-id> everyline-cli auth init' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F 'SESSION_ID=<same-session-id> everyline-cli auth complete' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" SKILL.md | grep -F 'firstInstall=true 且 authorizationRequired=true' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F 'auth init --restart --profile <profile> --as user --output json' >/dev/null
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F '`auth init` 返回 `reused=true`' >/dev/null
...
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | grep -F '[点击授权](<verification_uri_complete>)' >/dev/null
...
unzip -p "$output_dir/everyline-review-skill.zip" references/setup.md | grep -F 'EVERYLINE_DOUBAO_SKILLS_DIR' >/dev/null
...
unzip -p "$output_dir/everyline-review-skill.zip" references/setup.md | cmp - skills/everyline-review/references/setup.md
unzip -p "$output_dir/everyline-review-skill.zip" references/auth.md | cmp - skills/everyline-review/references/auth.md
test "$(unzip -Z1 "$output_dir/everyline-review-skill.zip" | wc -l | tr -d ' ')" = "4"
for entry in SKILL.md references/ references/setup.md references/auth.md; do
  unzip -Z1 "$output_dir/everyline-review-skill.zip" | grep -Fx "$entry" >/dev/null
done
test "$(unzip -Z1 "$output_dir/everyline-review-config-skill.zip")" = "SKILL.md"
